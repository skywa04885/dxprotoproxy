...
public abstract class NetInService extends NetService {
    ...
    public static abstract class Active extends NetService.Active {
         ...
         */
        private record NetConnHandle(@NotNull NetInConn conn, @NotNull Thread thread) {

        }

        private final @NotNull ServerSocket serverSocket;
        private final @NotNull LinkedList<NetConnHandle> connHandles;
            ...
            acceptingThread = new Thread(() -> {
                ...
                while (true) {
                    try {
                        ...
                        synchronized (connHandles) {
                            connHandles.add(connHandle);
                        }
                    } catch (Exception exception) {
        ...
        public void close(@NotNull NetConnHandle connHandle) throws Exception {
            ...
            assert connHandles.remove(connHandle);
        ...
        public void exit() throws Exception {
            ...
            while (!connHandles.isEmpty()) {
                close(connHandles.getFirst());
            }
        }
